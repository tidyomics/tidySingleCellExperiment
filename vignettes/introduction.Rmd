...
**Utilities**   |  
`as_tibble()`   | Convert to the feature-by-sample `tbl_df`
...
By default, the standard SingleCellExperiment print format is used. You can opt in to use the tidy tibble-style display, and opt out at any time. The setting can be configured for the current R session only, or saved to persist across sessions.
...
tidy_print_on()
pbmc_small_tidy  # Will display with tidy tibble-style format
```
...
To return to the standard SingleCellExperiment print format, use `tidy_print_off()`. By default, this only affects the current session:
...
```{r}
# Create a run column from the file path
pbmc_small_polished <-
    pbmc_small_tidy |>
    extract(file, "run", "../data/([a-z0-9]+)/outs.+", remove=FALSE)

# Reorder to have run column up front
pbmc_small_polished |>
...
pbmc_small_cell_type <-
    pbmc_small_UMAP |>
    left_join(cell_type_df, by=c(".sample"="cell"))

# Reorder columns
pbmc_small_cell_type |>
    select(.sample, first.labels, everything())
```
...
```{r}
# Add a per-cell mitochondrial abundance for the plot
pbmc_small_cell_type$mitochondrial <- rnorm(ncol(pbmc_small_cell_type))
